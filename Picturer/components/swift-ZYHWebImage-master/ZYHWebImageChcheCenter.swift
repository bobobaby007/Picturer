...
class ZYHWebImageChcheCenter: NSObject {
    class func readCacheFromUrl(url:NSString)->NSData?{
        var data:NSData?
        let path:NSString=ZYHWebImageChcheCenter.getFullCachePathFromUrl(url) as! NSString
    ...
    class func getFullCachePathFromUrl(url:NSString)->NSString{
        ...
        if !(fileManager.fileExistsAtPath(chchePath)) {
           // fileManager.createDirectoryAtPath(<#T##path: String##String#>, withIntermediateDirectories: <#T##Bool#>, attributes: <#T##[String : AnyObject]?#>)
            do {  try fileManager.createDirectoryAtPath(chchePath, withIntermediateDirectories: true, attributes: nil)
            }catch{
                print(error)
            }
        }
