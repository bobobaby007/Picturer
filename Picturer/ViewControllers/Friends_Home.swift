...
class Friends_Home: UIViewController, UITableViewDataSource, UITableViewDelegate, PicAlbumMessageItem_delegate{
    ...
    func setup(__frame:CGRect){
        ...
        _messageAlertView = UIView(frame: CGRect(x: 0, y: 10, width: _myFrame!.width, height: 40))
    ...
    func _refreshView(){
        ...
        if _hasNewMessage{
            _messageAlertView!.frame = CGRect(x: _myFrame!.width/2 - 100, y: 5, width: 200, height: 40)
        ...
        _tableView?.frame = CGRect(x: 0, y: 10+_messageH, width:  _myFrame!.width, height: _tableView!.contentSize.height)
