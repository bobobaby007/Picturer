...
class SocailHomeCell:UICollectionViewCell {
    ...
    override init(frame: CGRect) {
        ...
        _imgView=UIImageView(frame: CGRect(x: 30, y: 40, width: bounds.size.width-100, height: bounds.size.height-100))
        _imgView.contentMode=UIViewContentMode.ScaleAspectFill
        _imgView.layer.masksToBounds=true
        
        let _width=bounds.size.width/5
        _tag_view=UIImageView(frame: CGRect(x: bounds.size.width-_width-10, y:12, width: _width, height: _width))
        addSubview(_imgView)
